...
public class CreateAdminCommentHandler(IRepository<Comment> repository, IRepository<Feedback> feedbackRepository,IUserContext userContext)
  : ICommandHandler<CreateAdminCommentCommand, Result<int>>
{
  public async Task<Result<int>> Handle(CreateAdminCommentCommand request, CancellationToken cancellationToken)
  {
    ...
    var newComment = new Comment(request.comment, result.Id, commentId,request.filename);
